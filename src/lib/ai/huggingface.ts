...
const HF_TIMEOUT_MS = Number(process.env.HF_TIMEOUT_MS || 15000);

/** HF Inference Providers — enable at https://huggingface.co/settings/inference-providers */
export const HF_INFERENCE_PROVIDER = process.env.HF_INFERENCE_PROVIDER || 'hf-inference';

/** Defaults use models available on the free HF Inference provider */
export const HF_MODELS = {
  chat: process.env.HF_CHAT_MODEL || 'katanemo/Arch-Router-1.5B',
  chatFallbacks: [
...
export function hfChatCompletionOptions(
  model: string,
  ...
  return {
    model,
    provider: HF_INFERENCE_PROVIDER as 'hf-inference',
    messages,
...
export function withTimeout<T>(promise: Promise<T>, ms = HF_TIMEOUT_MS): Promise<T> {
  return Promise.race([
    promise,
    new Promise<T>((_, reject) => {
      setTimeout(() => reject(new Error(`HF request timed out after ${ms}ms`)), ms);
    }),
  ]);
}
...
export async function withModelFallback<T>(
  ...
  const timeoutMs = options?.timeoutMs ?? HF_TIMEOUT_MS;
  ...
  for (const model of models.slice(0, maxAttempts)) {
    try {
      ...
      console.warn(`[HF] ${model}: ${msg.slice(0, 120)}`);
    }
  }
    ...
    async (modelId) => {
      ...
      const out = await hf.chatCompletion(
        hfChatCompletionOptions(
          ...
          { maxTokens: options?.maxTokens ?? 280, temperature: options?.temperature ?? 0.65 },
        ),
      );
